...
<a name="kzb8-1739557201114"></a>[3.消息中间件](#bqty-1704437851800)
...
<a name="btj5-1739557201179"></a>[5.缓存写](#eqd9-1704732736281)
...
<a name="fbpl-1739557201245"></a>[.设计容错](#3nxv-1704757055271)
...
<a name="hqyu-1739557201307"></a>[适用措施：信号量（Semaphore）、条件变量（Condition Variable）。](#uzlr-1709309615599)
...
<a name="xzxt-1739557201371"></a>[专家系统ES](#bmeu-1705819204737)
...
<a name="eirf-1739557201438"></a>[需求定义](#4aeg-1706091062964)
...
<a name="acoc-1739557201504"></a>[.DHCP与DNS (★★★)](#imsh-1706198537828)
...
<a name="xgjo-1739557201568"></a>[如何让系统变得可靠：](#pqmh-1708523552230)
...
<a name="ie06-1739557201630"></a>[1.日志结构学派](#i55y-1644935943748)
...
<a name="ulkl-1739557201692"></a>[基于语句的复制(每个写入请求语句statment)](#k8yo-1708524004340)
...
<a name="xczv-1739557201755"></a>[分区主库1写入 A:你吃饭了吗？ 分区主库2写入B:没有。 分区1副本同步主库1落后分区2副本同步主库2    C读取1副本: 没有，后读取2副本。你吃饭了吗？](#7e9o-1708526727984)
...
<a name="otjt-1739557201818"></a>[环形，星型，all to all -mysql仅支持环形](#5feh-1708702903239)
...
<a name="ud6s-1739557201876"></a>[LLW 无法确定真实排序，强制排序，选择最新的最大时间戳，并丢弃较早的任何写入.Cassandra唯一支持的方式，Riak可选](#uzhw-1708874432788)
...
<a name="kovf-1739557201938"></a>[HBase Volde-mort为了减少实现复杂度放弃了次级索引。](#q1u5-1709803293988)
...
<a name="wueu-1739557202000"></a>[动态分区(适用键范围的分区)](#tkjw-1709800428033)
...
<a name="eqlh-1739557202058"></a>[7.事务](#7qvp-1708523954496)
...
<a name="wnfz-1739557202122"></a>[无主复制则是尽力而为：数据库将做尽可能多的事。错误时不会撤销已经完成的事。-错误恢复是应用程序的责任。](#0hze-1710252188073)
...
<a name="xvmg-1739557202185"></a>[但不能防止计数器增量的竞争状态。(读写非原子性，写依赖读的副本)](#ujzx-1710333397576)
...
<a name="7wmv-1739557202239"></a>[事务id决定它可以看到哪些对象，规则如下：](#ecrk-1710408160769)
...
<a name="hbxr-1739557202299"></a>[显式锁定(for update：读-改-写序列)](#vysa-1710172041914)
...
<a name="rwbu-1739557202353"></a>[显示锁定。 select ...fro update -锁定返回的所有行，锁定两个医生（行锁+间隙锁）](#crrt-1710437034816)
...
<a name="hvab-1739557202405"></a>[字面意义上的串行顺序执行事务](#mqa4-1710589545518)
...
<a name="pnqm-1739557202457"></a>[顺序执行事务使并发控制简单多，但吞吐限制为单机单核。吞吐量高的应用，单线程将可能成为严重瓶颈。](#rqu5-1710767939361)
...
<a name="rdhy-1739557202509"></a>[死锁频繁时重试造成浪费](#kmkr-1711207799940)

<a name="g287-1739557202511"></a>[谓词锁predicate lock](#ccq8-1710172621528)

<a name="hwur-1739557202513"></a>[谓词锁：属于符合搜索条件的对象，解决幻读phantoms](#kfpi-1711208306292)

<a name="2bde-1739557202515"></a>[事务A获取共享谓词锁shared-mode predicate lock读取匹配对象，与排它互斥](#6dqd-1711208459688)

<a name="ezfc-1739557202517"></a>[事务A写入需等待其他事务谓词锁释放。](#p3er-1711208588906)

<a name="py4e-1739557202519"></a>[适用于尚不存在，将可能添加的对象（幻象），如果两阶段包含谓词锁，可实现串行化。](#bdwj-1711208433268)

<a name="udgb-1739557202521"></a>[索引范围锁next-key lock](#zv5c-1710172626404)

<a name="y8s1-1739557202523"></a>[谓词锁性能不佳：活跃事务持锁多，检查匹配非常耗时](#a3ce-1711208973949)

<a name="ssta-1739557202525"></a>[2PL采用索引范围锁-简化版谓词锁：](#ayfb-1711209059199)

<a name="qsnq-1739557202527"></a>[索引位于roomid, 指示room123用于预定](#qmbu-1711212311152)

<a name="dnmj-1739557202529"></a>[或者索引位于时间上，指示12:00~13:00用于预定。](#ajbo-1711212362665)

<a name="ve5h-1739557202531"></a>[以上任何一条件，当另一个事务想写入时，需要更新索引-叶子节点，会收到共享锁。解决幻读和写入偏差](#k1ie-1711212755816)

<a name="wuwm-1739557202533"></a>[索引范围锁不如谓词锁精确，但开销低](#6lzs-1711212829915)

<a name="7twg-1739557202535"></a>[无可挂载索引，将退化为表锁，性能不利，但这是回退到一个安全位置。](#bofb-1711212877091)

<a name="k19k-1739557202537"></a>[序列化快照隔离SSI（高性能序列化算法）](#slwa-1710172573367)

<a name="pdbz-1739557202539"></a>[新算法：可序列化快照隔离SSI serializeble snapshot isolation 仅与快照隔离有很小性能损失. 08年 micheal cahill博士论文。](#lnci-1711213123166)

<a name="mhgf-1739557202541"></a>[用于单节点PostgreSQL9.1后序列化和分布式数据库](#weo0-1711213307841)

<a name="22s7-1739557202543"></a>[处于实践证明阶段](#ue1s-1711213377985)

<a name="volm-1739557202545"></a>[悲观pessimistic与乐观optimistic的并发控制](#yq9i-1710172639651)

<a name="l6zi-1739557202547"></a>[极致悲观：串行排它锁](#54jr-1711213690005)

<a name="er5x-1739557202549"></a>[序列化快照隔离SSI是乐观技术： ](#bjxf-1711213769533)

<a name="g6x4-1739557202551"></a>[缺点：多事务争用contention时性能不佳，大部分事务中止，吞吐量极限下有重试机制将使性能变差。](#mefw-1711213819265)

<a name="0rmn-1739557202553"></a>[优点：有备用容量，竞争不高场景比悲观好，可交换的原子操作可减少争用。](#nk1s-1711213915330)

<a name="q0bz-1739557202555"></a>[SSI在快照隔离基础上添加算法检测写入的序列化冲突，并决定中止哪些事务。](#snvf-1711214019984)

<a name="8nqo-1739557202557"></a>[基于过时前提的决策](#9lgw-1710172651041)

<a name="mt5b-1739557202559"></a>[医生值班读取-修改-写入。数据库检测过时并终止：](#td4m-1711214184809)

<a name="1say-1739557202561"></a>[检测对旧MVCC对象版本的读取(读之前存在未提交的写入)](#t08a-1711215992016)

<a name="qstj-1739557202563"></a>[检测影响先前读取的写入（读之后发生写入）](#aego-1711216030637)

<a name="cpcr-1739557202565"></a>[检测旧MVCC读取](#5smj-1710172660675)

<a name="uydk-1739557202567"></a>[事务提交时，检测是否有被忽略的写入已被提交。](#z7u2-1711216050316)

<a name="scto-1739557202569"></a>[SSI支持一致快照中长时间运行的读取-仅在提交时中止检测的陈旧读取。因为只读不产生幻读和写入偏差风险](#afqq-1711216243096)

<a name="ujzd-1739557202571"></a>[检测影响之前读取的写入](#mvok-1710172671381)

<a name="q6ns-1739557202573"></a>[事务写入提交时，在索引中查找最近读取受影响的其他事务，通知读取的事务非最新。并中止未提交写入的其他事务。](#mo5j-1711216452745)

<a name="gsfg-1739557202575"></a>[可序列化的快照隔离的性能](#0sq6-1710172685702)

<a name="0wcz-1739557202577"></a>[粒度granularity，精准哪些事务需要中止，但簿记开销显著。](#g2eg-1711216636788)

<a name="qtcc-1739557202579"></a>[简略的跟踪速度快（粗粒度），但导致不必要的事务中止。](#ux2m-1711216812849)

<a name="yl73-1739557202581"></a>[相比2PL, 不需要锁定阻塞，读性能高](#jzqb-1711216700614)

<a name="eqke-1739557202583"></a>[相比串行化，可扩展多台机器提高吞吐量，可跨分区读。](#qm51-1710756953994)

<a name="lbjj-1739557202585"></a>[SSI要求事务尽量短，避免长事务发生冲突中止。慢事务SSI比2PL或串行执行更不敏感](#wvsk-1711216911828)

<a name="bphl-1739557202587"></a>[8.分布式系统的麻烦](#enui-1704985776096)

<a name="7l1g-1739557202589"></a>[前几章failover故障切换, 复制延迟 与事务控制 处理各种可能发生的边缘情况，本章则是彻底的悲观总结。](#e2bp-1711387736669)

<a name="cbpg-1739557202591"></a>[故障与部分失效](#4yaw-1711386647072)

<a name="du9p-1739557202593"></a>[部分失效partial failure  不确定性的nondeterministic: IDC配电单元故障](#9d0g-1711387883443)

<a name="yqfo-1739557202595"></a>[云计算与超级计算机](#w7ln-1711386736921)

<a name="qgg9-1739557202597"></a>[不可靠的网络(Timeout)](#ynhs-1711386665148)

<a name="gxba-1739557202599"></a>[请求丢失： 拔网线](#fav6-1711388032544)

<a name="2qz5-1739557202601"></a>[请求排队：](#znsf-1711388048086)

<a name="fmfe-1739557202603"></a>[请求失效：](#gmdu-1711388084834)

<a name="3uk7-1739557202605"></a>[.....](#dssu-1711388054944)

<a name="szky-1739557202607"></a>[真实世界的网络故障](#y933-1711387087536)

<a name="m7hv-1739557202609"></a>[检测故障](#aywv-1711387099045)

<a name="4ioc-1739557202611"></a>[超时与无穷的延迟](#z4hi-1711387103669)

<a name="s6ka-1739557202613"></a>[级联失效cascading failure](#qs2e-1711388153448)

<a name="c00a-1739557202615"></a>[网络拥塞和排队](#gttn-1711387323883)

<a name="hbpq-1739557202617"></a>[TCP执行流量控制flow control](#v9f2-1711388199023)

<a name="ebjk-1739557202619"></a>[视频、语音会议使用UDP，折中丢包与流畅](#up7k-1711389082773)

<a name="wsjq-1739557202621"></a>[同步网络vs异步网络](#t56p-1711387113980)

<a name="1onj-1739557202623"></a>[有限延迟bounded delay：同步网络，不收排队影响](#fmrl-1711389283437)

<a name="qmre-1739557202625"></a>[我们不能简单地使用网络延迟可预测吗](#bwvp-1711387335617)

<a name="aogd-1739557202627"></a>[不可靠的时钟](#l3p1-1711386676597)

<a name="tdvn-1739557202629"></a>[机器的时钟：硬件设备，石英振荡器，](#faqp-1711469591901)

<a name="sxtx-1739557202631"></a>[不是完全准确的。](#xtjc-1711469629964)

<a name="2whn-1739557202634"></a>[同步时钟：网络时间协议NTP](#knwp-1711469656643)

<a name="fhoh-1739557202636"></a>[服务器从精确时间源获取时间-GPS接收器](#pfpq-1711469689783)

<a name="iaem-1739557202638"></a>[单调钟与时钟](#qoxv-1711387218692)

<a name="y2hf-1739557202640"></a>[时钟](#wu2k-1711387376627)

<a name="ayuq-1739557202642"></a>[Linux: clock_gettime(CLOCK_REALTIME)](#obwb-1711469781129)

<a name="0fsc-1739557202644"></a>[java:system.currentTimeMilis](#uxdp-1711469821742)

<a name="kkki-1739557202646"></a>[通常与NTP同步，但举例NTP服务器太远，则可能被强制重置(及时跳回）](#gwyx-1711469840905)

<a name="ebqk-1739557202648"></a>[单调钟](#hl1a-1711387385739)

<a name="iplu-1739557202650"></a>[保证总是前进](#uzaz-1711470052827)

<a name="5mga-1739557202652"></a>[Linux:CLOCK_MONOTONIC](#tbmn-1711469988760)

<a name="ihl4-1739557202654"></a>[java:System.nanoTime](#fb70-1711470014393)

<a name="59yy-1739557202656"></a>[调整频率：NTP检测到计算机石英钟更快或更慢，则调整单调钟频率（偏移skewing时钟）.](#sn42-1711470028035)

<a name="qlzy-1739557202658"></a>[不假定不同节点时钟存在同步：分布式系统用单调钟效果好，对测量的不准确不敏感](#ct3v-1711470667953)

<a name="adeu-1739557202660"></a>[时钟同步与准确性](#a7gy-1711387238026)

<a name="p0lw-1739557202662"></a>[漂移drifts: 计算机石英钟不准确原因,取决机器温度。](#pbyu-1711470750697)

<a name="lign-1739557202664"></a>[实现精确度：GPS接收机，精确时间协议PTP](#kgdy-1711470917769)

<a name="piqs-1739557202666"></a>[同步错误的原因：NTP守护进程配置错误，防火墙阻止NTP通信。](#kgez-1711470944635)

<a name="hoi7-1739557202668"></a>[依赖同步时钟](#vo0i-1711387245745)

<a name="ls2p-1739557202670"></a>[使用需要同步时钟的软件需监测所有机器间的时钟偏移](#zm2f-1711471000801)

<a name="wwv8-1739557202672"></a>[有序事件的时间戳](#fnxq-1711387475761)

<a name="lte8-1739557202674"></a>[写入排序问题：B客户端写入比A晚但具有更早的时间戳。 ](#n1pp-1711471283186)

<a name="yvib-1739557202676"></a>[ 最后写入为准LWW](#50kp-1711471399605)

<a name="uhme-1739557202678"></a>[ 版本向量](#axgx-1711471482711)

<a name="bw9q-1739557202680"></a>[时钟读数存在置信区间](#ofpq-1711387431462)

<a name="rxpo-1739557202682"></a>[ 置信区间的不确定性：NTP服务器的不去定性，加上服务器网络往返时间](#aihs-1711471513794)

<a name="iw25-1739557202684"></a>[当clock_gettime()返回值不会告诉你预期错误，不知道置信区间是5毫秒还是5年](#aave-1711471693187)

<a name="pqe7-1739557202686"></a>[例外：Spanner Google TureTime API明确报告本地时钟置信区间（最早，最晚）。](#znqm-1711471741654)

<a name="xdfw-1739557202688"></a>[全局快照的同步时钟](#bfz6-1711387449885)

<a name="lvhk-1739557202690"></a>[分布式的事务快照隔离，分布式事务ID时序成为站不住脚瓶颈](#dpco-1711471815493)

<a name="gqy2-1739557202692"></a>[Spanner通过置信区间来比较A,B发生的时序。](#2ihe-1711471939317)

<a name="zcul-1739557202694"></a>[分布式事务语义使用时钟同步是一个活跃的研究领域，没有在谷歌之外的主流数据库中实现。](#9qfk-1711471991096)

<a name="dclp-1739557202696"></a>[暂停进程](#3mnj-1711387250438)

<a name="wqxl-1739557202698"></a>[如何确保分布式主节点仍未领导者](#ui0e-1711472636558)

<a name="gn5r-1739557202700"></a>[方式一：从其他节点获得租约lease, 类似一个带超时的锁，周期性过期前续约。-和redison续约类似](#yh9a-1711472775035)

<a name="eur3-1739557202702"></a>[过期时，由另一个节点接管。](#kqwv-1711472837428)

<a name="vcdd-1739557202704"></a>[缺点：依赖同步时钟](#gi5a-1711473874947)

<a name="g3ge-1739557202706"></a>[意外的时间停顿：](#93uw-1711473992955)

<a name="v874-1739557202708"></a>[stop-the-world  HotSpot GC](#7kp7-1711474017909)

<a name="1p14-1739557202710"></a>[suspend虚拟机](#6ejp-1711474059672)

<a name="ohgh-1739557202712"></a>[随意暂停恢复：关闭笔记本盖子](#92se-1711474080346)

<a name="faii-1739557202714"></a>[窃取时间steal time:虚拟机切换或cpu上下文切换，在其他机器中花费的时间.](#tfxw-1711474128862)

<a name="xuwk-1739557202716"></a>[同步时的阻塞io：java惰性加载类文件；磁盘为网络文件系统或网络块设备-亚马逊EBS](#7jwg-1711474280466)

<a name="3hmc-1739557202718"></a>[磁盘交换内存抖动thrashing: 允许交换到磁盘（分页），内存访问可能导致页面错误page fault .话费大量时间将页面交换到内存，实际完成的工作很少thrashing。 服务器机器禁用页面调度，干掉进程释放内存。](#kvsa-1711474441136)

<a name="05zd-1739557202720"></a>[SIGSTOP暂停unix进程:Ctrl-Z立即阻止进程的CPU周期](#ocvk-1711474648160)

<a name="k2jz-1739557202722"></a>[这些都可以随时抢占preempt正在运行的线程。](#t1w7-1711474768157)

<a name="h6ku-1739557202724"></a>[响应时间保证](#cirf-1711473855327)

<a name="2h2d-1739557202726"></a>[硬实时hard real-time系统: 不满足特定截止时间deadline可能会导致整个系统故障。](#r1ji-1711474887529)

<a name="xr3n-1739557202728"></a>[嵌入式系统精心设计和测试：安全气囊不会因系统GC暂停弹出。](#pgjw-1711475001376)

<a name="vblw-1739557202730"></a>[大量的额外工作，昂贵。](#urp5-1711475089330)

<a name="cmrb-1739557202732"></a>[限制垃圾收集的影响](#i1wt-1711387521748)

<a name="6tyd-1739557202734"></a>[通知GC暂停，停止接收新请求-对延迟敏感的金融系统](#oxal-1711475134772)

<a name="fklp-1739557202736"></a>[变种：GC只收集新生代，积累老年代，一次重启一个节点，类似滚动升级。](#ho3v-1711475217820)

<a name="fxhd-1739557202738"></a>[知识、真相与谎言](#ujyf-1711386681865)

<a name="j8ct-1739557202740"></a>[真理由多数所定义](#hdwi-1711387268312)

<a name="rsll-1739557202742"></a>[stop the world 一分钟，被其他节点宣告死亡，重新选举。](#rxcy-1711561420884)

<a name="zjij-1739557202744"></a>[领导者与锁定](#3zea-1711387542106)

<a name="ezkt-1739557202746"></a>[数据库分区领导者只能有一个节点：避免脑裂](#b6am-1711561462773)

<a name="ysas-1739557202748"></a>[特定资源的锁和对象只允许一个事务/客户端持有](#3ynw-1711561525662)

<a name="oolg-1739557202750"></a>[用户唯一标识](#hqa3-1711561556083)

<a name="rkm6-1739557202752"></a>[节点认为它是天选者the choosen one](#rv2w-1711561875596)

<a name="u4zk-1739557202754"></a>[HBase例子：分布式锁的实现不正确，客户端1持有锁stop the world导致lease超时,客户端2获得锁进行处理，客户端1仍认为它具有有效的租约，即使它已经过期，从而破坏了存储的文件。](#gu25-1711561617808)

<a name="y3x6-1739557202756"></a>[防护令牌](#mhlf-1711387550649)

<a name="btlb-1739557202758"></a>[解决天选者问题：防护fencing](#gw5w-1711561950975)

<a name="bic4-1739557202760"></a>[fencing token :只允许增加屏蔽令牌的顺序进行写操作。](#vi5w-1711562021406)

<a name="tauf-1739557202762"></a>[服务器端检查token,获取锁时token增加33+1，存储服务器记录更高令牌34的写入，拒绝令牌33的请求。-一般是redisson续期，这个更像乐观锁。](#2yxl-1711562140276)

<a name="onqy-1739557202764"></a>[ZooKeeper将事务标识zxid和节点版本conersion用作屏蔽令牌。保证了单调递增](#e6n2-1711562077138)

<a name="tsrx-1739557202766"></a>[如文件存储服务器可将令牌卸载文件命中](#uujk-1711564755459)

<a name="qaim-1739557202768"></a>[拜占庭故障](#bsay-1711387279845)

<a name="uvqu-1739557202770"></a>[拜占庭故障Byzantine fault: 在不信任的环境中达成共识的问题称为拜占庭将军问题。](#id0b-1711628556885)

<a name="jbom-1739557202772"></a>[拜占庭将军问题：n位将军需要同意，一些叛徒通过发送虚假或不真实信息来混淆欺骗他人。](#lxw6-1711628612728)

<a name="iq31-1739557202774"></a>[拜占庭容错：系统在部分节点发生故障、不遵守协议，恶意攻击，扰乱网络时仍能继续正确工作：](#lpn6-1711628716132)

<a name="lzuu-1739557202776"></a>[航空航天环境计算机被辐射破坏，不可预知方式响应其他节点](#py6z-1711638469892)

<a name="siul-1739557202778"></a>[多个参与组织系统中，参与者试图欺骗他人。比特币等区块链对等网络让互不信任的各方同意交易不依赖中央当局。](#csli-1711638605446)

<a name="rqc9-1739557202780"></a>[弱谎言形式](#f38o-1711387557457)

<a name="hnil-1739557202782"></a>[添加防止撒谎弱形式机制；](#eepx-1711638744038)

<a name="r2vh-1739557202784"></a>[校验和防止数据包被破坏](#yseh-1711638821915)

<a name="9coh-1739557202786"></a>[公开访问的系统需仔细清理来自用户的任何输入](#2a8h-1711638847924)

<a name="bxcq-1739557202788"></a>[NTP多数对时间范围达成一致，配置错误的服务器将被排除](#zcdt-1711638888096)

<a name="ux9f-1739557202790"></a>[系统模型与实现](#x4lu-1711387293192)

<a name="vrez-1739557202792"></a>[定时假设三种系统模型：](#iyps-1711639179162)

<a name="fqbp-1739557202794"></a>[同步模型synchronous model: 网络延迟，暂停和时钟漂移不会超过某个固定上限；](#pgo0-1711640511485)

<a name="darj-1739557202796"></a>[部分同步模型partial synchronous：大多数情况像一个同步系统一样运行，有时出现超时，进程暂停和时钟漂移界限](#h1x4-1711640383000)

<a name="17tj-1739557202798"></a>[异步模型：一个算法不允许对实际做任何假设，事实上它甚至没有时钟。](#vkkn-1711640459345)

<a name="dagk-1739557202800"></a>[节点系统模型：-节点失效问题](#lg5f-1711640536166)

<a name="vtgf-1739557202802"></a>[崩溃停止crash-stop:只能由崩溃失效，且永远不会回来。](#ahco-1711640498506)

<a name="as5m-1739557202804"></a>[崩溃恢复crash-recovery: 具有稳定存储（非易失性磁盘），崩溃中保留，内存中状态会丢失-innodb三阶段提交](#35nl-1711640632666)

<a name="eace-1739557202806"></a>[拜占庭（任意）故障：节点可以做任何事情，包括戏弄和欺骗其他节点](#rpmc-1711640822813)

<a name="3nyh-1739557202808"></a>[具有崩溃恢复的部分同步模型是最有用的模型。分布式算法如何应用这种模型](#6gea-1711640879281)

<a name="ro8h-1739557202810"></a>[算法的正确性](#hwoz-1711387576180)

<a name="kglv-1739557202812"></a>[描述属性：定义算法的正确性](#i9kv-1711640972087)

<a name="8egk-1739557202814"></a>[锁生成屏蔽令牌，要求具有以下属性：](#qsck-1711641049261)

<a name="8tfg-1739557202816"></a>[唯一性：没有两个屏蔽令牌请求返回相同的值](#tvxj-1711641079676)

<a name="ymsf-1739557202818"></a>[单调序列：时序实现单调](#2d0a-1711641105012)

<a name="btx2-1739557202820"></a>[可用性：请求防护令牌且不会崩溃的节点，最终会收到响应](#mlbd-1711641130527)

<a name="yynn-1739557202822"></a>[安全性和活性](#1lz8-1711387585629)

<a name="ccs5-1739557202824"></a>[澄清这种情况，需区分两种不同性质](#hctm-1711641203049)

<a name="nomd-1739557202826"></a>[安全性safety: 唯一性和单调序列](#k48b-1711641295367)

<a name="jibn-1739557202828"></a>[活性：可用性](#fmy7-1711641313609)

<a name="obml-1739557202830"></a>[将系统模型映射到现实世界](#ndiw-1711387592922)

<a name="crzl-1739557202832"></a>[9.一致性共识](#rp9s-1705080859004)

<a name="aer8-1739557202834"></a>[共识consensus: 让所有节点对某件事达成一致](#vrvh-1712996687843)

<a name="haek-1739557202836"></a>[脑裂brain split: 两个节点都认为自己是master, 经常导致数据丢失；](#oqcs-1712996726634)

<a name="serg-1739557202838"></a>[一致性保证](#obqn-1711730044966)

<a name="2l6y-1739557202840"></a>[最终一致性：收敛convergence, 最终收敛到相同的值](#ku5g-1712996774138)

<a name="txgn-1739557202842"></a>[事务一致性：避免同时执行事务导致的竞争状态](#ga60-1712996859001)

<a name="jluq-1739557202844"></a>[分布式一致性模型：面对延迟和故障时，如何协调副本间的状态](#eovi-1712996916251)

<a name="o0a6-1739557202846"></a>[线性一致性](#thyh-1711730049653)

<a name="tn42-1739557202848"></a>[背后的想法：原子一致性atomic consistency, 强一致性strong consistency, 立即一致性 immediate consistency ,外部一致性 external consistency](#bvre-1712997165569)

<a name="rooe-1739557202850"></a>[看起来只有一个数据副本，实际有多个副本 - 强一致性就是逆分布式化，看成一个整体。](#qbsw-1712997247742)

<a name="nsw8-1739557202852"></a>[线性一致是一个新鲜度保证 recency gurantee;](#f0su-1712997377059)

<a name="pknu-1739557202854"></a>[非线性一致的例子： world cup ,  alice refresh - DB1 return germany 1: 0 argentina won ;  bob refresh website - DB2(un asychronize) return still playing;](#mtb7-1712997407717)

<a name="tbgd-1739557202856"></a>[为什么使得系统线性一致？](#qimr-1711730403998)

<a name="0p8x-1739557202861"></a>[例子：CAS使得多个客户端并发按序更新](#ffat-1712998155207)

<a name="4wxy-1739557202863"></a>[线性一致性与可序列化是两种完全不同的保证：](#3elk-1712997586736)

<a name="avze-1739557202865"></a>[可序列化serializeble: 单对象或者多对象操作，确保按照某种顺序执行](#qpvu-1712998010030)

<a name="kw4c-1739557202867"></a>[线性一致性Linearizability:是读取和写入寄存器（单个对象）的新鲜度保证，不会将操作组合为事务，不会阻止写入偏差等问题](#ojsh-1712998125336)

<a name="xysc-1739557202869"></a>[单副本强可串行性strong-1SR: 一个数据库可提供可串行性和线性一致, 基于两阶段锁2PL和实际串行执行通常是线性一致的](#5hda-1712998332148)

<a name="alrj-1739557202871"></a>[SSI一致性快照不是线性一致的：不会包括比快照更新的写入；-不会阻塞非写的读。](#obfn-1712998432352)

<a name="7lbv-1739557202873"></a>[依赖线性一致性](#bje8-1711730427157)

<a name="bj1c-1739557202875"></a>[锁定与领导选举](#xyll-1711730652620)

<a name="ziwg-1739557202877"></a>[领导选举的一种方法：每个节点启动时尝试获取锁，成功者称为领导者。](#2fqs-1712998599719)

<a name="fnxp-1739557202879"></a>[保证锁时线性一致的： zookeeper 和 etcd 协调服务的一致性算法。curator基于zookeeper上更高级别的配方](#cbzr-1712998723133)

<a name="wvun-1739557202881"></a>[约束和唯一性保证](#gpva-1711730659968)

<a name="kqcz-1739557202883"></a>[这种情况类似于一个锁，锁定用户名。该操作与原子性的cas类似：](#9jmj-1712998930168)

<a name="om7y-1739557202885"></a>[用户名和邮箱；](#lhyk-1712999021176)

<a name="qmuk-1739557202887"></a>[银行账号不为负数；](#7sgo-1712999031257)

<a name="zfbg-1739557202889"></a>[航班剧院预定了同一时间同一位置](#so5k-1712999045093)

<a name="bris-1739557202891"></a>[实际应用中，处理这些限制有时可以接受。如调换位置并补偿。](#q05j-1712999070223)

<a name="ivtc-1739557202893"></a>[跨信道的时序依赖](#7cbv-1711730671963)

<a name="uhw7-1739557202895"></a>[将用户上传的照片降低分辨率以加快下载速度：](#cqkp-1712999245541)

<a name="sh2w-1739557202897"></a>[步骤：消息队列一般针对较短的消息而设计，一张图片可能达到几M.所以步骤为](#3zoo-1712999362806)

<a name="ema6-1739557202899"></a>[1.上传照片-  web server-2.  (store img) 存储文件服务器  ](#umkb-1712999318589)

<a name="jsst-1739557202901"></a>[  | 3. send message  || 5.fetch full size img  6.store resized img](#er8u-1712999518585)

<a name="hlmz-1739557202903"></a>[消息队列   - 4.deliver message     image resizer](#iiv4-1712999532781)

<a name="n7mx-1739557202905"></a>[问题：存储和缩放存在两个不同的信道，没有线性一致的新鲜度保证](#0yys-1712999647965)

<a name="t0my-1739557202907"></a>[3和4可能比存储服务器内部的复制更快，当5时，可能会看到图像旧版本或者什么都没有，如果处理的是旧版本，则产生不一致。](#xtun-1712999768959)

<a name="0dbp-1739557202909"></a>[实现线性一致性的系统](#ytt0-1711730433598)

<a name="8qy4-1739557202911"></a>[单主复制（可能线性一致）protential潜在线性一致：](#j997-1712999879559)

<a name="5gfz-1739557202913"></a>[快照隔离并发错误；](#t174-1712999979364)

<a name="y277-1739557202915"></a>[脑裂；异步复制延迟，故障切换](#hh6o-1712999996439)

<a name="c6r2-1739557202917"></a>[共识算法（线性一致）：共识算法可以安全实现线性一致性的存储，zookeeper 和 etcd](#z3nq-1713000050923)

<a name="monv-1739557202919"></a>[多主复制（非线性一致）：同时多个节点上处理写入，异步复制到其他节点，会产生冲突的写入](#q0kz-1713000066034)

<a name="srwt-1739557202921"></a>[无主复制（也许不是线性一致的）：Dynamo风格，法定人数读写获得强一致性。通常不完全正确](#ca4m-1713030167882)

<a name="gzon-1739557202923"></a>[线性一致性和法定人数](#9gst-1711730681011)

<a name="vuwh-1739557202925"></a>[最安全的做法是：假设采用Dynamo风格的无主复制的系统是不能提供线性一致性。](#sqvx-1713030399799)

<a name="bpqk-1739557202927"></a>[线性一致性的代价](#vpxf-1711730440465)

<a name="ayrm-1739557202929"></a>[CAP定理](#fqqf-1711730717308)

<a name="bihw-1739557202931"></a>[不仅是单主多主复制的后果，任何线性一致性的数据库都有这个问题：](#8hzl-1713030564813)

<a name="yeut-1739557202933"></a>[.应用需要线性一致性，副本因为网络问题断开连接，请求必须等到网络问题解决或直接返回错误（不可用unavailable）](#7lux-1713030592466)

<a name="vftv-1739557202935"></a>[.网络断开也可独立处理请求（如多主复制），可以在网络问题前保持可用，但其行为不是线性一致的。](#48wk-1713031150235)

<a name="ecoj-1739557202937"></a>[CAP定理没有帮助：网络分区是不可避免的，所以只能是AP 或 CP。](#lsdc-1713031249676)

<a name="hhux-1739557202939"></a>[线性一致性和网络延迟](#kxsm-1711730728177)

<a name="pvjk-1739557202941"></a>[ 线性一致的系统惊人的少。例如多核CPU并没有保证一定能读到第一个线程写入的值（除非使用内存屏障memory barrier或围栏fence）](#lbl2-1713031508671)

<a name="gfdh-1739557202943"></a>[ 牺牲线性一致性的原因是性能performance. 内存访问先走缓存，副本都是异步更新的，所以失去了线性一致性。](#pnsn-1713031585885)

<a name="ptrj-1739557202945"></a>[搞笑的线性一致性存储不可实现。](#8y6v-1713031685808)

<a name="yvmg-1739557202947"></a>[顺序保证](#on5f-1711730057760)

<a name="mxne-1739557202949"></a>[写入顺序order of write: 单主复制的主要目的就是复制日志中确定写入顺序。](#dhiv-1713031758875)

<a name="uyxo-1739557202951"></a>[序列顺序serial order：某种序列顺序some sequential order](#cyfr-1713031815356)

<a name="e4sy-1739557202953"></a>[顺序与因果](#z48k-1711730472679)

<a name="ooof-1739557202955"></a>[顺序有助于保持因果关系causality](#khzl-1713031939113)

<a name="opgs-1739557202957"></a>[一致前缀读：先看到答案再看到问题。违背因cause果effect. 存在因果依赖causal dependency](#nbes-1713031966982)

<a name="3ums-1739557202959"></a>[更新不存在的记录：3位领导者间的复制，一些写入会压到其他写入。一条记录需被创建才能更新。](#xehj-1713032060272)

<a name="fsvq-1739557202961"></a>[检测并发写入：A和B并发，则不知道顺序，A和B不知道彼此。](#v1kv-1713032135082)

<a name="5h9u-1739557202963"></a>[快照隔离：写偏差read skew，读取的数据违反因果关系（不可重复读）](#ozi8-1713032226341)

<a name="adcu-1739557202965"></a>[事务的写偏差write skew：幻读和值班。](#pgzq-1713032264605)

<a name="1zzc-1739557202967"></a>[世界杯球赛结果，跨信道的时序依赖。](#3ang-1713032756197)

<a name="ja65-1739557202969"></a>[因果一致causally：系统服从因果关系锁规定的顺序。](#tryg-1713032788961)

<a name="dry3-1739557202971"></a>[因果顺序不是全序的](#niwx-1711730751513)

<a name="habc-1739557202973"></a>[全序total order: 自然数更大或更小，总是有序的](#x8gk-1713086054855)

<a name="rkvg-1739557202975"></a>[偏序partially order: 一个集合大于另一个集合，但是其它情况下无法比较](#wvsa-1713086222914)

<a name="rijg-1739557202977"></a>[线性一致性：操作是全序的，系统表现好像只有一个副本，所有操作都是原子性的。意味对每个操作总能判定发生顺序](#u7jq-1713086312064)

<a name="rp8i-1739557202979"></a>[因果性：两个操作没在彼此之前发生，即并发无法比较顺序，意味着因果关系定义了一个偏序，而不是全序。一些有序一些无序](#xa0w-1713086399916)

<a name="mymv-1739557202981"></a>[线性一致的数据存储中是不存在并发操作的。确保每个请求都是在唯一时间线上自动处理](#uaxz-1713086531138)

<a name="reki-1739557202983"></a>[并发意味着时间线会分叉然后合并。操作的偏序：](#qs64-1713086635361)

<a name="aoyh-1739557202985"></a>[类似git系统，一个提交commit发生在另一个提交之后。分支合并merge会在这些并发创建的提交相融合时创建。](#ctkd-1713086670170)

<a name="2wgp-1739557202987"></a>[线性一致性强于因果一致性](#qdnf-1711730766239)

<a name="gmaj-1739557202989"></a>[线性一致性隐含implies着因果关系;](#dam4-1713086279684)

<a name="tfag-1739557202991"></a>[捕获因果关系](#uqqx-1711730776113)

<a name="snvp-1739557202993"></a>[为了维持因果性，需确认操作发生在哪个操作之前happened before:](#agrw-1713086796608)

<a name="n7e9-1739557202995"></a>[刑侦决定Y时是否知到X与Y因果关系](#rys3-1713086865582)

<a name="2nlw-1739557202997"></a>[检测并发写入：推广版本向量解决因果依赖](#znup-1713086911030)

<a name="bq0z-1739557202999"></a>[SSI序列化隔离快照：事务提交时，检测读取是否最新，为此跟踪哪些事务被那些事务锁读取。](#1ivk-1713086936754)

<a name="k2so-1739557203001"></a>[序列号顺序](#jsuf-1711730485869)

<a name="tnqx-1739557203003"></a>[序列号sequence number或时间戳timestamp](#y9ti-1713091524670)

<a name="xnzb-1739557203005"></a>[非因果序列号生成器](#qpya-1711730796494)

<a name="xce2-1739557203007"></a>[兰伯特时间戳](#mtzj-1711730811106)

<a name="m7mp-1739557203009"></a>[兰伯特时间戳提供了与因果关系一致的总排序：施行一个全序](#f8uv-1713091653199)

<a name="hqeh-1739557203011"></a>[检测并发写入有着不同目的：区分两个操作是并发的，一个因果依赖另一个。](#2ayx-1713091789731)

<a name="byra-1739557203013"></a>[光有时间戳排序还不够](#elc6-1711730816628)

<a name="ejdd-1739557203015"></a>[全序广播](#aw2l-1711730490423)

<a name="zohq-1739557203017"></a>[可靠交付reliable delivery：没有消息丢失，如果消息被传递到一个节点，它将被传递到所有节点](#axul-1713091937508)

<a name="tgoz-1739557203019"></a>[全序交付totally ordered delivery: 消息以相同的顺序传递给每个节点。](#cruc-1713091989510)

<a name="xyoh-1739557203021"></a>[使用全序广播](#hp3a-1711730834892)

<a name="lsjc-1739557203023"></a>[zookeeper和etcd共识服务实际上实现了全序广播](#ztws-1713108978601)

<a name="y4b3-1739557203025"></a>[状态机复制state machine repliacation: 每个副本都按相同的顺序处理相同的写入。](#i8bk-1713109007327)

<a name="4hxr-1739557203027"></a>[与之类似，使用全序广播来实现可序列化的事务：真的串行执行，确定的事务以存储过程来执行，每个节点以相同的顺序处理这些消息。](#8jch-1713109071990)

<a name="mupq-1739557203029"></a>[以及防护令牌的锁服务，全序广播使得所有消息按顺序一次编号，序列号可当做防护令牌(单调递增)，zookeeper中这个序列号称为zxid;](#w19a-1713109204107)

<a name="jmw1-1739557203031"></a>[使用全序广播实现线性一致的存储](#i3vg-1711730844593)

<a name="tg1e-1739557203033"></a>[使用线性一致性存储实现全序广播](#xlae-1711730855140)

<a name="4qa8-1739557203035"></a>[分布式事务与共识](#y1bv-1711730380233)

<a name="srxo-1739557203037"></a>[让几个节点达成一致get serveral nodes to agree on somthing](#yt5r-1713109388776)

<a name="iwgp-1739557203039"></a>[领导选举：避免脑裂](#d7t7-1713109479427)

<a name="8aiv-1739557203041"></a>[原子提交atomic commit：多节点事务提交/中止/回滚](#frsp-1713109508353)

<a name="1l2e-1739557203043"></a>[原子提交与两阶段提交2PC](#abk6-1711730509647)

<a name="s1hj-1739557203045"></a>[原子性可以避免中间态，多对象事务原子性确保二级索引与主数据库保持一致。](#0h2f-1713110484428)

<a name="7ln2-1739557203047"></a>[从单节点到分布式原子提交](#iqfd-1711730882368)

<a name="ykkd-1739557203049"></a>[从数据库的角度看，这是单独的事务，任何跨事务正确性的保证都是应用自己的问题。](#h1ym-1713110537245)

<a name="pcsg-1739557203051"></a>[2PC两阶段提交简介](#roa9-1711730906391)

<a name="8zvf-1739557203053"></a>[coodinator协调子事务，子事务单独执行完，在准备阶段询问是否可提交](#vaax-1705118249954)

<a name="qph1-1739557203055"></a>[.全部为是，则分别提交。](#7zwc-1705118288968)

<a name="vfdq-1739557203057"></a>[.其中有否，则中断回滚。](#3b8s-1705118337513)

<a name="vztk-1739557203059"></a>[系统承诺](#kpgk-1711730913393)

<a name="zaje-1739557203061"></a>[一旦协调者做出决定，这一决定是不可撤销的。](#wkui-1713110739239)

<a name="luq1-1739557203063"></a>[协调者失败](#qhdj-1711730921387)

<a name="y7sf-1739557203065"></a>[参与者投赞成票后，参与者协调者崩溃，数据库1不知道是否提交或中止。](#jpd8-1713110804532)

<a name="cvt2-1739557203067"></a>[三阶段提交](#ixzv-1711730924556)

<a name="lm4a-1739557203069"></a>[阻塞blocking原子提交协议, 2PC可能卡主等待协调者恢复的情况。](#edp3-1713111179295)

<a name="yghw-1739557203071"></a>[在提交阶段，如果子事务进程crash,或者协调者崩溃的时的解决方案，弱原子性，最终一致性。](#llyc-1711731148564)

<a name="ytdm-1739557203073"></a>[许多coodinator不支持高可用或者只支持基本的复制](#nlrk-1705118500709)

<a name="aspb-1739557203075"></a>[实践中的分布式事务](#xexf-1711730524691)

<a name="exbh-1739557203077"></a>[云服务由于其导致的运维问题而选择不实现分布式事务。](#inqh-1713111548114)

<a name="deni-1739557203079"></a>[两阶段难以实现安全保证，且导致的运维问题造成性能下降。](#zuho-1713111590325)

<a name="txxz-1739557203081"></a>[数据库内部的分布式事务](#gf92-1711731287296)

<a name="ozvr-1739557203083"></a>[VoltDB 与mysql Cluster 的NDB引擎就有这样的内部事务支持。](#jhqw-1705080950920)

<a name="s3nf-1739557203085"></a>[异构分布式事务](#gvng-1713111648038)

<a name="zbi6-1739557203087"></a>[保证异构数据库的事务更有挑战性](#st60-1713111659854)

<a name="0ey3-1739557203089"></a>[恰好一次的消息处理](#btdt-1711730944613)

<a name="nou6-1739557203091"></a>[XA事务eXtended Architecture](#qvbk-1711730954739)

<a name="scjw-1739557203093"></a>[怀疑时持有锁](#med0-1711730959684)

<a name="hxol-1739557203095"></a>[从协调者故障中恢复](#ouyq-1711730969007)

<a name="p2hd-1739557203098"></a>[唯一的出路是让管理员手动决定提交还是回滚事务](#ouur-1713116418144)

<a name="s7ea-1739557203100"></a>[分布式事务的限制](#gzya-1711730975747)

<a name="onxf-1739557203102"></a>[协调者本身就是一种数据库，存储了事务的结果。](#jkt3-1713116480399)

<a name="61b4-1739557203104"></a>[协调者默认不是高可用的，只有基本的复制](#mcvv-1713116529836)

<a name="gf61-1739557203106"></a>[应用服务器不再是无状态的了](#h6gt-1713116550997)

<a name="iusq-1739557203108"></a>[不能检测不同系统间的死锁，且无法与SSI协同工作](#rrrl-1713116568769)

<a name="eoax-1739557203110"></a>[与构建容错系统的目标背道而驰，分布式事务有扩大失效amplifying failures的趋势](#wbzw-1713116600081)

<a name="jyiu-1739557203112"></a>[容错共识](#npne-1713118985168)

<a name="y0py-1739557203114"></a>[一个或多个节点提议propose某些值，共识算法决定decides采用其中的某个值。](#beiq-1713118985169)

<a name="smrr-1739557203116"></a>[一致同意Uniform agreement:  没有两个节点的决定不同-核心 决定了相同的结果，一旦决定不能改变](#pq0x-1713118985170)

<a name="mcfc-1739557203118"></a>[完整性Integrity: 没有节点决定两次-核心](#xnwp-1713118985171)

<a name="iuy4-1739557203120"></a>[有效性validity: 如果一个节点决定了值v, 则v由某个节点所提议-排除平凡的解决方案](#2bty-1713118985172)

<a name="bkj0-1739557203122"></a>[终止Termination由所有为崩溃的几点来最终决定值。-活性属性，必须取得进展，即使部分节点出现故障，其他节点也必须达成一致。不超过一般的节点崩溃或不可达](#9k32-1713118985173)

<a name="uavz-1739557203124"></a>[稳健达成共识：只要少于三分之一的节点存在拜占庭故障](#scne-1713118985174)

<a name="mygb-1739557203126"></a>[共识算法和全序广播](#prxr-1713118985175)

<a name="jesg-1739557203128"></a>[视图戳复制VSR viewstamped repliacation, Paxos，Raft, Zab](#zmcg-1713118985176)

<a name="dyfd-1739557203130"></a>[全序广播相当于重复进行多轮共识：](#1u8f-1713118985177)

<a name="53iz-1739557203132"></a>[一致同意属性，所有节点决定以相同的顺序传递相同的消息](#d2l2-1713118985178)

<a name="omgs-1739557203134"></a>[完整性属性，消息不会重复](#z7u4-1713118985179)

<a name="3e88-1739557203136"></a>[有效性属性，消息不会被破坏，也不能凭空编造](#pphb-1713118985180)

<a name="tzxz-1739557203138"></a>[终止属性,消息不会丢失](#gqmq-1713118985181)

<a name="huhl-1739557203140"></a>[视图戳复制，raft和zab直接实现了全序广播，比重复一次一致one value a time的共识更高效， paxos 下这种优化称为multi-paxos.](#lxll-1713118985182)

<a name="qikh-1739557203142"></a>[单领导者复制和共识](#krsz-1713118985183)

<a name="kxzl-1739557203144"></a>[时代编号和法定人数](#90tt-1713118985184)

<a name="h7ea-1739557203146"></a>[共识的局限性](#qywj-1713118985185)

<a name="dhkd-1739557203148"></a>[成员与协调服务](#8qzt-1713118985186)

<a name="57o5-1739557203150"></a>[将工作分配给节点](#4tlb-1713118985187)

<a name="3u2f-1739557203152"></a>[服务发现](#tbbj-1713118985188)

<a name="tzjf-1739557203154"></a>[成员服务](#1lld-1713118985189)

<a name="tdch-1739557203156"></a>[第三部分：衍生数据](#46k8-1705118453932)

<a name="nxdt-1739557203158"></a>[记录和衍生数据系统](#rboc-1716226529015)

<a name="gsnz-1739557203160"></a>[从高层次看，存储和处理数据的系统可以分为两大类](#cmdh-1716226737947)

<a name="lwis-1739557203162"></a>[记录系统System of record](#1dmv-1716226552894)

<a name="lmd5-1739557203164"></a>[真相源source of truth. 记录用户输入新数据，标准化normalized](#dh96-1716226777418)

<a name="hrtc-1739557203166"></a>[衍生数据系统Derived data system](#qjlf-1716226568395)

<a name="ugys-1739557203168"></a>[缓存cache, 衍生数据是冗余的，重复了已有的信息。](#igtl-1716226871151)

<a name="k6qi-1739557203170"></a>[10.批处理](#0bgb-1716226496297)

<a name="lu6c-1739557203172"></a>[服务（在线系统）：高可用性，性能指标-响应时间](#yygb-1720542683707)

<a name="u3jp-1739557203174"></a>[批处理系统（离线系统）：定时任务job，性能指标-吞吐量](#goo7-1720542700859)

<a name="937j-1739557203176"></a>[流处理系统（准实时系统）：介于在线离线系统之间](#mv2a-1720542952432)

<a name="6i8w-1739557203178"></a>[使用Unix工具的批处理](#6lu4-1716228538928)

<a name="lqqt-1739557203180"></a>[分析简单日志](#hyii-1716228597300)

<a name="svzd-1739557203182"></a>[Unix命令链：网站上找到五个最受欢迎的网页（nginx/access.log）](#smoq-1720543218731)

<a name="dnb7-1739557203184"></a>[命令链与自定义程序Ruby](#ukyi-1716228816049)

<a name="erus-1739557203186"></a>[排序 VS 内存中的聚合](#cnuw-1716228829632)

<a name="69oc-1739557203188"></a>[工作集working set(作业需要随机访问的内存大小)，单个url 重复出现百万次，哈希表性能好](#siiu-1720543940755)

<a name="7bdh-1739557203190"></a>[工作集大于内存，排序优，与SSTables 和 LSM树一样，内存聚合归并排序，GNU 中的sort通过溢出至磁盘自动应该大于内存大额数据集，瓶颈可能是磁盘io。-MQ的死信处理也是溢出到磁盘。](#lms0-1720544023798)

<a name="l5ja-1739557203192"></a>[Unix哲学](#mgei-1716228613042)

<a name="w58b-1739557203194"></a>[统一的接口](#sk3b-1716228852580)

<a name="xuod-1739557203196"></a>[Unix中统一的接口是一个文件file](#33ju-1720874904091)

<a name="3poq-1739557203198"></a>[ASCII文本，\n 分隔，](#yogq-1720875513301)

<a name="z2hl-1739557203200"></a>[CSV， ，号分隔](#7fvb-1720875542091)

<a name="m9vl-1739557203202"></a>[逻辑与布线相分离](#3oxq-1716228869387)

<a name="x35e-1739557203204"></a>[松耦合loose coupling](#0zy9-1720876020643)

<a name="qgc6-1739557203206"></a>[晚期绑定late binding](#226j-1720876035472)

<a name="mihy-1739557203208"></a>[控制反转inversion of control](#zedb-1720876065554)

<a name="dxdk-1739557203210"></a>[透明度与实验](#ftj8-1716228879278)

<a name="pyvn-1739557203212"></a>[MapReduce和分布式文件系统](#djdy-1716228554334)

<a name="btao-1739557203214"></a>[MapReduce像是分布在数前台机器上的Unix。](#illh-1720876369554)

<a name="lble-1739557203216"></a>[Hadoop的Map-Reduce视线中，文件系统成为HDFS,基于google文件系统GFS的开源实现。](#e1cy-1720876447842)

<a name="euaj-1739557203218"></a>[Amazon S3，  Azure blob ](#mapk-1720876955975)

<a name="y1mh-1739557203220"></a>[MapReduce作业执行](#kdge-1716228625984)

<a name="wbeq-1739557203222"></a>[Mapper在每条输入记录上调用一次,从输入记录中提取键值（提取键值）](#vleh-1720877349422)

<a name="3stm-1739557203224"></a>[Reduce拉取Mapper生成的键值对，收集属于同一个键的所有值。（收集同键的所有值）](#aol7-1720877397533)

<a name="zmrn-1739557203226"></a>[分布式执行MapReduce](#kv6u-1716231603620)

<a name="uosr-1739557203228"></a>[即在不同服务器上执行mapper reduce 的task。](#wy7r-1720877483243)

<a name="fkgx-1739557203230"></a>[MapReduce工作流](#d0qe-1716231619319)

<a name="vmro-1739557203232"></a>[workflow。一系列命令，输出写入临时文件，下一个命令从临时文件中读取-物化中间状态](#rvme-1720877910897)

<a name="1uep-1739557203234"></a>[Reduce端连接与分组](#pyoc-1716228643855)

<a name="r6tu-1739557203236"></a>[用户活动事件](#1pza-1716231634942)

<a name="hybq-1739557203238"></a>[左侧是事件日志/活动事件activity events/点击流事件clickstream evnets](#4l72-1720878002631)

<a name="dwpo-1739557203240"></a>[右侧是用户数据库](#hivt-1720878131008)

<a name="rbau-1739557203242"></a>[用户行为日志与用户档案的连接](#cvii-1720878237531)

<a name="na3c-1739557203244"></a>[原始:逐个遍历活动事件关联查询。缺点：吞吐量受限数据库服务器且容易压垮](#eiqe-1720878248508)

<a name="6qsl-1739557203246"></a>[改良：获取用户数据库副本（ETL进程从数据库备份中提取数据，参考数仓），放在同一个分布式文件系统中。将用户数据库存储HDFS中的一组文件中，用户活动记录存储另一组文件中，](#iueg-1720878322010)

<a name="xmoz-1739557203248"></a>[并能用MapReduce将左右相关记录集中到同一个地方进行高效处理。](#ruua-1720878498786)

<a name="9zkt-1739557203250"></a>[排序合并连接](#fihy-1716231663765)

<a name="pew1-1739557203252"></a>[1.map提取键值(userid-url  usrid-do)和reduce(userid all url do)](#q51q-1720947527716)

<a name="nuk5-1739557203254"></a>[2.二次排序secondary sort ：对1的合并按时间戳排序](#4kvv-1720947648039)

<a name="fnqs-1739557203256"></a>[排序合并连接sort-merge join: reduce一次处理一个特定用户所有记录，只需将一条用户记录保存在内存中，无需发送网络请求](#j8br-1720947805879)

<a name="s1ka-1739557203258"></a>[把相关数据放在一起](#8vrk-1716231670998)

<a name="7ing-1739557203260"></a>[Mapper和排序过程确保特定用户id连接操作的必须数据都放在同一个地方](#t1tc-1720947947656)

<a name="bvj7-1739557203262"></a>[GROUP BY](#9j4s-1716228658160)

<a name="7xuy-1739557203264"></a>[除了连接之外，“把相关数据放在一起”的常见模式是记录分组。SQL中的Group By](#a6wg-1720952083180)

<a name="wzzj-1739557203266"></a>[处理倾斜](#qa24-1716231688547)

<a name="ylpx-1739557203268"></a>[名流热点数据hot spot](#f55j-1720952155251)

<a name="dgkj-1739557203270"></a>[参考负载倾斜和消除热点](#julg-1720952222343)

<a name="7ox8-1739557203272"></a>[Mapper将热键的关联记录随机发送到几个Reduce之一](#akfb-1720952286891)

<a name="juln-1739557203274"></a>[Map端连接](#2tox-1716228671225)

<a name="jbb3-1739557203276"></a>[预处理输入数据](#qiil-1720971178915)

<a name="6n3o-1739557203278"></a>[广播散列连接](#wjhj-1716231713037)

<a name="fs80-1739557203280"></a>[broadcast hash join](#k3se-1720971260954)

<a name="sjbc-1739557203282"></a>[分区散列连接](#su27-1716231723170)

<a name="rtto-1739557203284"></a>[Map桶端连接bucketd map jons](#5h9a-1720971514797)

<a name="ybcf-1739557203286"></a>[Map端合并连接](#ftdq-1716231736611)

<a name="zmkk-1739557203288"></a>[MapReduce工作流与Map端连接](#yrmc-1716231744413)

<a name="ozto-1739557203290"></a>[批处理工作流的输入](#zzir-1716228678980)

<a name="b0a1-1739557203292"></a>[OLTP ：查找少量记录，使用索引，呈现给用户](#hiqn-1720971586510)

<a name="mxun-1739557203294"></a>[OLAP：扫描大量记录，执行分组与聚合，输出有着报告的形式，商业决策的分析。](#cqba-1720971612883)

<a name="2swz-1739557203296"></a>[建立搜索索引](#bwjf-1716231761454)

<a name="llf9-1739557203298"></a>[MapReduce最初作为google搜索引擎的索引](#lypn-1720971694941)

<a name="3k2s-1739557203300"></a>[全文搜索和模糊索引  Lucene](#iijq-1720971776882)

<a name="hlma-1739557203302"></a>[分区和二级索引](#vcun-1720971828838)

<a name="ry2b-1739557203304"></a>[键值存储作为批处理输入](#8fix-1716231782091)

<a name="n7j0-1739557203306"></a>[批处理输入的哲学](#audd-1716231802005)

<a name="azvt-1739557203308"></a>[视为不可变且避免副作用：](#6i34-1720972317458)

<a name="w0i4-1739557203310"></a>[人类容错human fault tolerance](#zciu-1720972319614)

<a name="oqhp-1739557203312"></a>[最小化不可逆性mininizing irreversibility](#oolm-1720972319615)

<a name="2u8g-1739557203314"></a>[Hadoop与分布式数据库的对比](#lp33-1716228684462)

<a name="br4b-1739557203316"></a>[Hadoop 像Unix的分布式版本，HDFS是它的文件系统，而MapReduce是Unix进程的怪异实现](#g0vj-1720972341966)

<a name="8iwy-1739557203318"></a>[MapReduce论文发表前，并行连接算法已在数据库实现-大规模并行处理MPP massively parallel processing ](#gjbt-1720972427948)

<a name="z6zo-1739557203320"></a>[Gamma database machine, Teradata, Tandem NonStop SQL](#go7m-1720972523351)

<a name="chsi-1739557203322"></a>[区别：MMP专注一组及骑行并行执行分析SQL 查询](#5mnr-1720972551600)

<a name="c1gd-1739557203324"></a>[MapReduce和分布式文件系统的组合是一个可以运行任意程序的通用操作系统](#gx7b-1720972580058)

<a name="l8ew-1739557203326"></a>[存储多样性](#eebl-1716231817367)

<a name="skyc-1739557203328"></a>[数据湖data lake / 企业数据中心enterprise data hub](#5lga-1720972673599)

<a name="irbl-1739557203330"></a>[Hadoop 常用于实现数仓ETL Extract, Transform, Load](#jrhw-1720972777529)

<a name="2l1s-1739557203332"></a>[处理模型多样](#ysfv-1716231826972)

<a name="gvgu-1739557203334"></a>[针对频繁故障设计](#kwz4-1716231831752)

<a name="gbzp-1739557203336"></a>[MapReduce之后](#p0zx-1716228568687)

<a name="qtoq-1739557203338"></a>[建立在MapReduce上的高级编程模型](#wcyn-1721117329960)

<a name="cdxj-1739557203340"></a>[Pig，Hive, Cascading, Crunch](#l9ug-1721117359714)

<a name="b98z-1739557203342"></a>[流处理：加速批处理的另一种方法](#bz2s-1721117576614)

<a name="byqz-1739557203344"></a>[物化中间状态](#fvak-1716228706372)

<a name="xyk5-1739557203346"></a>[中间状态intermediate state](#npb7-1721117756510)

<a name="zmsq-1739557203348"></a>[物化materialization:中间态写入文件的过程](#0rlu-1721117876651)

<a name="dfhv-1739557203350"></a>[MapReduce完全物化中间态的方法不足：](#fnqf-1721117967688)

<a name="qbez-1739557203352"></a>[只有在前驱作业生成其输入完成时才能启动](#pood-1721117984973)

<a name="cro1-1739557203354"></a>[mapper通常是多余的：仅仅读取由Reducer写入的同样文件](#iljf-1721118032959)

<a name="whuq-1739557203356"></a>[临时文件（数据）被复制到多个节点](#tghz-1721118071785)

<a name="xcit-1739557203358"></a>[解决：数据流引擎](#aer2-1721118148615)

<a name="rgff-1739557203360"></a>[数据流引擎](#sdac-1716231877023)

<a name="mauq-1739557203362"></a>[分布式批处理新引擎：](#4gu5-1721118111495)

<a name="pgqw-1739557203364"></a>[spark,  Flink: 独立的网络通信层，调度器，用户向API](#nfnv-1721118162692)

<a name="jhau-1739557203366"></a>[Tez : 薄库，依赖YARN shuffle服务实现节点间的数据复制](#0kbz-1721123564974)

<a name="zrs9-1739557203368"></a>[dataflow engines模型:](#jojo-1721118227522)

<a name="eoed-1739557203370"></a>[函数/算子operators: 一个算子的输出连接到另一个算子的输入](#kstp-1721118246090)

<a name="0syx-1739557203372"></a>[优点：](#zbrh-1721118327524)

<a name="xr1x-1739557203374"></a>[排序等昂贵工作只需在实际需要地方执行，不是默认在Map和Reduce阶段间](#8oqo-1721118330975)

<a name="riep-1739557203376"></a>[没有不必要的Map任务](#amzf-1721118375240)

<a name="yccg-1739557203378"></a>[连接和数据依赖都是显示声明的](#bunw-1721118382779)

<a name="yjh2-1739557203380"></a>[算子中间态足以保存内存或写入本地磁盘，比HDFS需要更少I/O](#bzui-1721122836028)

<a name="p5hk-1739557203382"></a>[算子可在输入就绪后立即执行，不许等待前驱完成](#ka7f-1721123164712)

<a name="rtsf-1739557203384"></a>[MapReduce为每个任务启动新JVM, 而df enginer可重用现有JVM，减少启动开销](#rmr3-1721123191298)

<a name="uynw-1739557203386"></a>[执行速度更快](#mp4i-1721123431562)

<a name="6v9j-1739557203388"></a>[使用：](#2cxo-1721123304059)

<a name="cfqq-1739557203390"></a>[算子是Map和Reduce的泛化/抽象，相同处理代码可在任意引擎上运行](#vyfa-1721123442398)

<a name="jqlc-1739557203392"></a>[Pig Hive Casacding中实现的工作流无需修改代码，修改配置，从MapReduce切换到Tez 或spark.](#qxck-1721123483308)

<a name="qwyf-1739557203394"></a>[容错](#glfr-1716231882229)

<a name="7tj5-1739557203396"></a>[重启，算子重新计算](#neqa-1721499456929)

<a name="ajgh-1739557203398"></a>[关于物化的讨论](#npsg-1716231884238)

<a name="pudk-1739557203400"></a>[图与迭代处理](#dvkh-1716228727874)

<a name="6xre-1739557203402"></a>[图数据模型常用于机器学习应用](#y476-1721499725472)

<a name="oiv3-1739557203404"></a>[图形算法PageRank:根据连接到某个网页的其他网页来估计该网页的流行度](#ld62-1721499760165)

<a name="2rj2-1739557203406"></a>[闭包传递transitive clousure: 重复跟进标明地点归属关系的边，生成数据库中北美地点列表](#et5r-1721499798288)

<a name="dwe5-1739557203408"></a>[MapReduce低效：没考虑算法迭代性](#x4m1-1721499995671)

<a name="waxd-1739557203410"></a>[Pregel处理模型](#lf5y-1716231892007)

<a name="wgmg-1739557203412"></a>[图批处理的优化：批量同步并行（BSP）计算模型。](#1faf-1721499959178)

<a name="2asv-1739557203414"></a>[Pregel模型：Google的Pregel论文推广了处理图的方法。Apache Giraph , Spark GraphX API 和 Flink Gelly API](#3mh8-1721500149918)

<a name="gn1f-1739557203416"></a>[容错](#pxnd-1716231909468)

<a name="imhd-1739557203418"></a>[并行执行](#v8gr-1716231912877)

<a name="0t2y-1739557203420"></a>[高级API和语言](#cxqk-1716228736349)

<a name="b9qh-1739557203422"></a>[Hive,pig, Cascading,Crunch等高级语言和API, Tez](#qlnr-1721841646688)

<a name="isx5-1739557203424"></a>[向声明式查询语言的转变](#dfa6-1716231923208)

<a name="nf1r-1739557203426"></a>[声明式declarative](#zman-1721841903902)

<a name="pamf-1739557203428"></a>[与SQL的完全声明式查询有很大的区别：](#inea-1721841971310)

<a name="h5wr-1739557203430"></a>[围绕回调函数的概念建立，自由调用任意代码来决定输出什么。](#ahki-1721842021736)

<a name="v49o-1739557203432"></a>[可基于大量已有库的生态系统创作： 解析、自然语言分析，图像分析以及运算数值算法或统计算法等。](#bdyf-1721842047384)

<a name="o0iy-1739557203434"></a>[专业化的不同领域](#m2pu-1716231937991)

<a name="obk6-1739557203436"></a>[空间算法：最近邻搜索K-nearest neghbors](#wpot-1721842227412)

<a name="ivun-1739557203438"></a>[11.流处理](#iwmv-1721931597264)

<a name="7giu-1739557203440"></a>[流处理Stream processing: 当事件发生时就可以立即处理](#yeid-1722103475505)

<a name="ohgt-1739557203442"></a>[传递事件流](#08iq-1721844360659)

<a name="ev71-1739557203444"></a>[事件event：流处理的上下文件记录](#n0th-1722103658739)

<a name="gh4a-1739557203446"></a>[生产者producer/发布者publisher/发送者sender](#titb-1722103707652)

<a name="sxkt-1739557203448"></a>[消费者consumer/订阅者subscribers/接收者recipients](#yhbb-1722103745716)

<a name="xiv4-1739557203450"></a>[主题topic/流stream：相关事件的聚合](#jdqp-1722103794393)

<a name="qk59-1739557203452"></a>[通知机制：数据库触发器trigger(算是弥补机制)、消息传递系统messaging system](#2buw-1722103875913)

<a name="bh4w-1739557203454"></a>[消息系统](#ipga-1721932434160)

<a name="m0g0-1739557203456"></a>[两个关键问题：](#qm3o-1722104029796)

<a name="kiuy-1739557203458"></a>[1.生产者快于消费者：丢掉消息/将消息放入缓冲队列/背压backpressure(流量控制flow control) 阻塞生产者，以免其发送更多消息](#sztq-1722104046748)

<a name="svmf-1739557203460"></a>[2.节点崩溃或脱机，是否有消息丢失： 持久性，高吞吐和低延迟则接收丢失](#5ycq-1722104289674)

<a name="h1xh-1739557203462"></a>[直接从生产者传递给消费者](#cj2t-1721932544748)

<a name="w0hg-1739557203464"></a>[直接网络通信，而不通过中间节点：](#hjhp-1722104612024)

<a name="qivq-1739557203466"></a>[UPD组播，金融行业，低延时，不可靠，应用层协议可恢复丢失的包（生产者必须记住发送的包，按需重新发送）](#0ggy-1722104631748)

<a name="1g4t-1739557203468"></a>[无代理的消息库，ZeroMQ、nanomsg, TCP或IP多播](#hnj8-1722104735615)

<a name="xoyr-1739557203470"></a>[StatsD 和 Brubeck 使用UDP](#pcd7-1722104800545)

<a name="njql-1739557203472"></a>[消费者在网络上公开了服务，生产者可以直接发送HTTP和RPC请求将消息推送给使用者webhooks.](#3se2-1722104863261)

<a name="545f-1739557203474"></a>[消息代理](#jwkh-1721932555905)

<a name="kox9-1739557203476"></a>[消息代理message broker(也称为消息队列message queue)](#dtlb-1722106424439)

<a name="89gh-1739557203478"></a>[消息代理与数据库对比](#fyus-1721932559957)

<a name="gnik-1739557203480"></a>[.数据存储：数据库保留数据直至显示删除；大多数消息代理在消息成功传递给消费者时地总删除](#aj3x-1722106519844)

<a name="zrcw-1739557203482"></a>[.删除消息快，所以队列可以很短。如过缓冲很多消息可能会溢出到磁盘，吞吐量会下降](#pkoh-1722106591225)

<a name="hpwx-1739557203484"></a>[.匹配方式：数据库通常支持二级索引和各种搜索数据的方式；消息代理按照模式匹配订阅子集](#lfst-1722106688956)

<a name="w9pr-1739557203486"></a>[.最新数据：数据库通常支持事务，基于某个时间点数据快照；消息代理不支持任意查询，有新消息会通知客户端](#q13l-1722106755764)

<a name="xisa-1739557203488"></a>[多个消费者](#v0lh-1721932568637)

<a name="yuqm-1739557203490"></a>[负载均衡load balance: 多个客户端从同一个队列中消费。JMS中称为共享订阅shared subscripiton](#enlv-1722191268259)

<a name="ldak-1739557203492"></a>[扇出fan-out:将每条消息传递给多个消费者](#bjqd-1722191397355)

<a name="oxeq-1739557203494"></a>[确认与重新交付](#aoas-1721932573289)

<a name="tu6r-1739557203496"></a>[消息代理使用确认acknowlegements:客户端需显示告知代理消息处理完毕的时间，代理将消息从队列中移除。](#8lsc-1723155930519)

<a name="ns0l-1739557203498"></a>[确认丢失：需一种原子提交协议才能处理，如分布式事务。](#w6ac-1723156157534)

<a name="qsdl-1739557203500"></a>[负载均衡与重传的组合不可避免导致消息顺序(JMS，AMQP标准要求保留消息的顺序。)重新排序](#zlnz-1723156224601)

<a name="4zyo-1739557203502"></a>[解决：每个消费者使用单独的队列-即不适用负载均衡功能](#fovj-1725028626326)

<a name="vjop-1739557203504"></a>[消息完全独立，消息重排序并不是个问题，消息之间存在因果依赖关系则是个重要问题](#3kzh-1725028667923)

<a name="b4v4-1739557203506"></a>[分区日志](#9bmj-1721932438668)

<a name="kkc4-1739557203508"></a>[消息传递/不留痕迹：网络数据包 和 消息代理 (可抓包和日志持久)](#xree-1725028818863)

<a name="kzzv-1739557203510"></a>[持久化：数据库和文件系统，显式删除前将永久记录](#sbbo-1725028858585)

<a name="qh0o-1739557203512"></a>[消息代理log-base message brokers: 既有数据库的持久化，又有消息传递的低延迟通知。](#n6lq-1725028981222)

<a name="rgeu-1739557203514"></a>[使用日志进行消息存储](#keft-1721932590686)

<a name="ixfw-1739557203516"></a>[生产者通过将消息追加到日志末尾来发送消息，消费者通过以此读取日志来接收消息。](#tlnm-1725029031868)

<a name="0p3u-1739557203518"></a>[Unix tail -f 监视文件被追加写入的数据就是这样工作的。](#1606-1725029123561)

<a name="mb3l-1739557203520"></a>[Apache Kafka, Amazon Kinesis Streams , Twitter DistributedLog都是基于日志的消息代理](#aq8x-1725029525384)

<a name="noun-1739557203522"></a>[Google Cloud Pub/Sub在架构上类似，单对外暴露的是JMS风格的API，而不是日志抽象](#0wz5-1725029598472)

<a name="dayb-1739557203524"></a>[尽管写入磁盘，但通过跨多台机器分区，每秒能实现数百万消息的吞吐量，并通过复制来实现容错性。](#zqkl-1725029648581)

<a name="slwb-1739557203526"></a>[日志与传统消息相比](#pr6y-1721932607757)

<a name="gsnw-1739557203528"></a>[天然支持扇出式消息传递：过个消费者独立读取日志，而不相互影响](#i3q5-1725115574888)

<a name="3lyy-1739557203530"></a>[负载平衡：读取消息不会从日志中删除，代理可以将整个分区分配给消费者组中的节点，而不是单挑消息分配给客户端。](#ymqj-1725115609372)

<a name="alpd-1739557203532"></a>[消费者偏移量](#o0ft-1721932621047)

<a name="pcb1-1739557203534"></a>[和单领导者数据库复制中常见的日志序列号非常相似，代理无需跟踪每条消息，只需定期记录消费者偏移即可。有助于提高基于日志的系统吞吐量](#emow-1725115793168)

<a name="kgmk-1739557203536"></a>[问题：消费者节点失效而没有记录偏移量，重启后这些消息将被处理两次。本章后续讨论。](#gmal-1725115911246)

<a name="3y7c-1739557203538"></a>[磁盘空间使用](#ipmx-1721932629330)

<a name="berb-1739557203540"></a>[只追加写入日志，回收时将旧段删除或移动到归档存储](#5ep6-1725115987947)

<a name="vljs-1739557203542"></a>[问题，消费者跟不上生产者，指向删除段，错过一些消息。](#kjgm-1725116313754)

<a name="weib-1739557203544"></a>[日志实现一个有限大小的缓冲区：循环缓冲区circular buffer/环形缓冲区，填满时将丢弃旧消息](#k3hi-1725116359622)

<a name="kzlj-1739557203546"></a>[设 大型硬盘容量为 6TB, 顺序写入为150MB/s， 最快需要约11小时填满磁盘，之后开始覆盖旧消息。实践中的部署很少能用满磁盘的写入带宽。所以通常可保留一个几天甚至几周的日志缓冲区](#jjma-1725116440115)

<a name="s4tc-1739557203548"></a>[以上保留多长时间的消息，日志吞吐量或多或少保持不变，因为每个消息都写入磁盘。](#bdoa-1725116521841)

<a name="juox-1739557203550"></a>[   而默认将消息保存在内存中，当队列很短，系统很快；当系统开始写入磁盘，就慢得多，所以吞吐量取决于保留的历史数量。](#rxx7-1725116637183)

<a name="wv4w-1739557203552"></a>[当消费者更不上生产者时](#91b6-1721932634947)

<a name="6sxx-1739557203554"></a>[3种选择：丢弃信息、进行缓冲或施加背压](#dztq-1725116764093)

<a name="blse-1739557203556"></a>[巨大运维优势：丢失消息仅影响当前消费者，不会中断其它消费者，可进行调试测试，不必中断生产服务。](#mgyh-1726767519795)

<a name="iovz-1739557203558"></a>[重播旧信息](#6tnb-1721932645175)

<a name="d9ol-1739557203560"></a>[删除消息：AMQP和JMS消息代理](#yygs-1726767645074)

<a name="ftgq-1739557203562"></a>[只读操作：基于日志的代理。](#u81b-1726767864887)

<a name="wy44-1739557203564"></a>[更像上一章节的批处理。允许更多的实验操作，通过偏移量重播。](#opy5-1726767877078)

<a name="vfwo-1739557203566"></a>[流与数据库](#rnwj-1721932350699)

<a name="ngqt-1739557203568"></a>[保持系统同步](#ehyq-1721932450135)

<a name="gsaa-1739557203570"></a>[变更数据捕获](#1ako-1721932460819)

<a name="5mdb-1739557203572"></a>[变更数据捕获的实现](#xkpl-1721932670593)

<a name="z2fg-1739557203574"></a>[初始快照](#uxpn-1721932683846)

<a name="bo89-1739557203576"></a>[日志压缩](#j0w6-1721932690312)

<a name="lcbn-1739557203578"></a>[变更流的API支持](#5pqr-1721932694290)

<a name="8owv-1739557203580"></a>[事件溯源](#qkmx-1721932472062)

<a name="cclo-1739557203582"></a>[从时间日志中派生出当前状态](#vydq-1721932714045)

<a name="qkck-1739557203584"></a>[命令和事件](#xbwr-1721932732154)

<a name="czka-1739557203587"></a>[状态，流和不变形](#k3v8-1721932742549)

<a name="2kww-1739557203589"></a>[从同一事件日志中派生多个视图](#diql-1721932751200)

<a name="xabm-1739557203591"></a>[并发控制](#tzcl-1721932772042)

<a name="f1vd-1739557203593"></a>[不变性的限制](#urmz-1721932775107)

<a name="zzi8-1739557203595"></a>[流处理](#nbcs-1721932419014)

<a name="as8a-1739557203597"></a>[流处理的应用](#jf4x-1721932507881)

<a name="3nat-1739557203599"></a>[复合时间处理](#80rp-1721932792357)

<a name="pgv8-1739557203601"></a>[流分析](#6jjt-1721932799443)

<a name="fau5-1739557203603"></a>[维护物化视图](#5iog-1721932801692)

<a name="uboh-1739557203605"></a>[在流上搜索](#0asi-1721932807278)

<a name="a8us-1739557203607"></a>[消息传递和RPC](#10bm-1721932817553)

<a name="ksgw-1739557203609"></a>[时间推理](#oca5-1721932519502)

<a name="byzl-1739557203611"></a>[事件时间和处理时间](#is6l-1721932844579)

<a name="ejyu-1739557203613"></a>[知道什么时候准备好](#aue5-1721932851975)

<a name="3yya-1739557203615"></a>[你用的是谁的时钟](#vsi1-1721932859473)

<a name="guls-1739557203617"></a>[窗口的类型](#gryf-1721932868260)

<a name="yfkd-1739557203619"></a>[流式连接](#jbet-1721932523864)

<a name="4ccj-1739557203621"></a>[流流连接(窗口连接)](#eaux-1721932880085)

<a name="5n89-1739557203623"></a>[流表连接(流扩展)](#hixz-1721932915139)

<a name="jkry-1739557203625"></a>[表表连接(维护物化视图)](#zbsv-1721932945841)

<a name="west-1739557203627"></a>[连接的时间依赖性](#xqtb-1721932962310)

<a name="hank-1739557203629"></a>[容错](#v1om-1721932530009)

<a name="lldo-1739557203631"></a>[微批量与存档点](#pqfe-1721932996846)

<a name="rvyy-1739557203633"></a>[原子提交再现](#elio-1721933013244)

<a name="71pt-1739557203635"></a>[幂等性](#bwyd-1721933023622)

<a name="xe8b-1739557203637"></a>[失败后重建状态](#dijm-1721933026909)

<a name="rqdl-1739557203639"></a>[12. 数据系统的未来](#vrhs-1739552413424)

<a name="y9su-1739557203641"></a>[数据集成](#ede5-1739555478378)

<a name="60eo-1739557203643"></a>[组合使用衍生数据的工具](#72md-1739555630486)

<a name="jwtp-1739557203645"></a>[理解流数据](#ws0u-1739555668111)

<a name="vupq-1739557203647"></a>[衍生数据与分布式事务](#zqyv-1739555677949)

<a name="8xrf-1739557203649"></a>[全局有序的限制](#sdr6-1739555687850)

<a name="r1in-1739557203651"></a>[排序事件以捕捉因果关系](#zk4p-1739555695102)

<a name="farv-1739557203653"></a>[批处理与流处理](#niva-1739555642700)

<a name="wlaz-1739557203655"></a>[维护衍生状态](#r4mh-1739555731398)

<a name="iros-1739557203657"></a>[应用演化后重新处理数据](#dx0m-1739555748166)

<a name="4stt-1739557203659"></a>[Lambda架构](#2vm7-1739555772929)

<a name="eig6-1739557203661"></a>[统一批处理和流处理](#6flf-1739555780198)

<a name="txqb-1739557203663"></a>[分拆数据库](#10z2-1739555560574)

<a name="qdrc-1739557203665"></a>[组合使用数据存储技术](#8wfw-1739555819342)

<a name="s5is-1739557203667"></a>[创建索引](#havr-1739555874428)

<a name="ak7c-1739557203669"></a>[一切的元数据库](#ep2e-1739555885454)

<a name="sduj-1739557203671"></a>[开展分拆工作](#am3j-1739555961678)

<a name="qvf5-1739557203673"></a>[分拆系统和集系统](#trtv-1739555969624)

<a name="mq0f-1739557203675"></a>[少了什么？](#lcy6-1739555980725)

<a name="dmtp-1739557203677"></a>[围绕数据流设计应用](#97j6-1739555836550)

<a name="ov3r-1739557203679"></a>[应用代码所谓衍生函数](#gyfk-1739555997047)

<a name="3klj-1739557203681"></a>[应用代码的状态和分离](#2njs-1739556023517)

<a name="38en-1739557203683"></a>[数据流：应用代码与状态变化的交互](#pgmn-1739556032065)

<a name="xmd8-1739557203685"></a>[流处理器和服务](#fyub-1739556046567)

<a name="amt4-1739557203687"></a>[观察衍生数据状态](#dmmo-1739555844651)

<a name="byjw-1739557203689"></a>[物化视图和缓存](#48oq-1739556059722)

<a name="nm6j-1739557203691"></a>[有状态，可离线的客户端](#srii-1739556068892)

<a name="xhhf-1739557203693"></a>[将状态变更推送给客户端](#zdkz-1739556083230)

<a name="zcy7-1739557203695"></a>[端到端的事件流](#70bj-1739556096239)

<a name="7ufv-1739557203697"></a>[读也是事件](#k9nk-1739556109784)

<a name="hqo3-1739557203699"></a>[多分区处理数据](#rknv-1739556117246)

<a name="gtgo-1739557203701"></a>[将事情做正确](#ydeg-1739555534326)

<a name="etfp-1739557203703"></a>[为数据库使用端到端的参数](#bool-1739556139461)

<a name="ihyg-1739557203705"></a>[正好执行一次操作](#utq9-1739556271989)

<a name="hkv6-1739557203707"></a>[抑制重复](#mxfy-1739556285089)

<a name="lbxa-1739557203709"></a>[操作标识符](#9kug-1739556293281)

<a name="qoib-1739557203711"></a>[在数据系统中应用端到端的思考](#dc4e-1739556300938)

<a name="v0no-1739557203713"></a>[强制约束](#cai0-1739556155120)

<a name="ngcz-1739557203715"></a>[唯一性的约束要达成共识](#jeqp-1739556425758)

<a name="3ftq-1739557203717"></a>[基于日志消息传递中的唯一性](#lcq0-1739556436827)

<a name="eteu-1739557203719"></a>[多分区请求处理](#dk4a-1739556687945)

<a name="ficy-1739557203721"></a>[及时性与完整性](#dbka-1739556159893)

<a name="vhzv-1739557203723"></a>[数据流系统的正确性](#bmr3-1739556705098)

<a name="zpcf-1739557203725"></a>[宽松地解释约束](#loih-1739556715504)

<a name="ujcb-1739557203727"></a>[无协调数据系统](#v94l-1739556733932)

<a name="kzfq-1739557203729"></a>[信任但验证](#boru-1739556172269)

<a name="8p4u-1739557203731"></a>[维护完整性，尽管软件有bug](#zgrh-1739556749129)

<a name="vnkq-1739557203733"></a>[不要盲目信任任何承诺](#irhz-1739556777111)

<a name="aszc-1739557203735"></a>[验证的文化](#1d8z-1739556789067)

<a name="r3ur-1739557203737"></a>[为可审计性而设计](#61de-1739556792067)

<a name="zg6k-1739557203739"></a>[端到端原则重现](#guck-1739556800394)

<a name="nvyh-1739557203741"></a>[利用可审计数据系统的工具](#tjmv-1739556812856)

<a name="wfga-1739557203743"></a>[做正确的事情](#dizf-1739555546308)

<a name="gzs4-1739557203745"></a>[预测性分析](#va2h-1739556847417)

<a name="6ltd-1739557203747"></a>[偏见与分析](#c1wq-1739556910451)

<a name="kfkz-1739557203749"></a>[责任与问责](#ijng-1739556917694)

<a name="ebon-1739557203751"></a>[反馈循环](#thso-1739556928624)

<a name="mm2e-1739557203753"></a>[隐私和追踪](#gt9r-1739556854431)

<a name="vfpx-1739557203755"></a>[监视](#qol8-1739556939696)

<a name="yj1g-1739557203757"></a>[同意与选择的自由](#zgzn-1739556948482)

<a name="qacj-1739557203759"></a>[隐私和数据使用](#ewvo-1739557089382)

<a name="0ghf-1739557203761"></a>[数据资产与权力](#vlsj-1739557096182)

<a name="xsf5-1739557203763"></a>[记着工业革命](#muzc-1739557108489)

<a name="1g6t-1739557203765"></a>[立法与法律](#7mjx-1739557115858)

<a name="dvkh-1739557203767"></a>[本章小结](#hmh3-1739556886643)
